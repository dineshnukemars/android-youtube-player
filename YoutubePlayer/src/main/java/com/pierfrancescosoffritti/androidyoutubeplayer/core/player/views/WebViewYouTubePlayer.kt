 ...
 */
class WebViewYouTubePlayer constructor(
    context: Context,
    attrs: AttributeSet? = null,
    defStyleAttr: Int = 0
) : WebView(context, attrs, defStyleAttr), YouTubePlayer,
...
//         if the video's thumbnail is not in memory, show a black screen
        webChromeClient = object : WebChromeClient() {
            ...
            override fun getDefaultVideoPoster(): Bitmap? {
                ...
                Log.d("WebViewYouTubePlayer", "getDefaultVideoPoster ${result?.height}")

                return result ?: Bitmap.createBitmap(1, 1, Bitmap.Config.RGB_565)
            }

            override fun onPermissionRequest(request: PermissionRequest?) {
                Log.d("WebViewYouTubePlayer", "onPermissionRequest")
            }


            override fun onPermissionRequestCanceled(request: PermissionRequest?) {
                Log.d("WebViewYouTubePlayer", "onPermissionRequestCanceled")
            }
        }
    }
